...
async def get_user_info(ctx: RunContextWrapper[User]) -> str:
    ...
    if id == 1:
        ...
        user_info = "User name is Akash. He is 30 years old. He is a doctor by profession. He likes mountains."
    else:
...
agent = Agent[User](
    name="Assistant",
    instructions=(
        "You are an expert of agentic AI. "
        "When the user asks about themselves or personal details, always call the `get_user_info` tool "
        "using the provided user_id in context instead of guessing."
    ),
    tools=[get_user_info]
)
